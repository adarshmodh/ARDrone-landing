...
cv::KalmanFilter KF(3, 3, 2);
//cv::Point mousePos;
//GetCursorPos(&mousePos)

// intialization of KF...
KF.transitionMatrix = *(cv::Mat_<float>(3, 3) << 0,0,0,   0,0,0,	0,0,0);
cv::Mat_<float> control(3,1); control.setTo(Scalar(0));
cv::Mat_<float> measurement(3,1); measurement.setTo(Scalar(0));

KF.statePre.at<float>(0) = est_x;
KF.statePre.at<float>(1) = est_y;
KF.statePre.at<float>(2) = est_z;

cv::setIdentity(KF.measurementMatrix);
cv::setIdentity(KF.processNoiseCov, Scalar::all(1e-3));
...
while(ros::ok())
{
 // First predict, to update the internal statePre variable
	control(0) = drone_navdata.vx;
	control(1) = drone_navdata.vy;
	control(2) = 0;
 ...
 estdata.y = estimated.at<float>(1);
 estdata.z = estimated.at<float>(2);
